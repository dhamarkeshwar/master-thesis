\begin{figure}[ht]
    ...
    \begin{tcolorbox}[title=\textbf{$\Lambda_{RO}$ \cite{cryptoeprint:2025/576}}, width=1.2\textwidth, colframe=blue!75!black, colback=blue!10, sharp corners]
        ...
        \textbf{Initialization:}
            ...
            $P_i$ registers their public key $PK_i$ in the public ledger and all agree on $PK_0\neq g$, a commitment 
            key or a public key whose secret key is known to a target person.
        ...
        \begin{itemize}
            ...
            \item For each $1\leq i\leq n$, $D$ computes $f(i)=f_i$ and uses $PK_i$ to encrypt the secret share 
              $PK_i^{f_i}=y_i$. $D$ also encrypts(/commits) to the secret $g^{f_0}$ using the encryption(/commitment) 
